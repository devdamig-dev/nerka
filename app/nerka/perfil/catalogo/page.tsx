    ...
    <main className="px-4 py-5 pb-24 lg:px-8 lg:py-8 lg:pb-10">
      <div className="mb-4 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <Link href="/niar" className="rounded-xl border border-[#E6DDD0] bg-white p-2 text-[#6E7F63]">
            <ArrowLeft size={14} />
          </Link>
          <div>
            <h1 className="text-xl font-semibold text-[#2F3A2B] lg:text-2xl">Mi catálogo</h1>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-4">
        {products.map((p) => (
          <article key={p.id} className="overflow-hidden rounded-2xl border border-[#E6DDD0] bg-white">
            <img src={p.image} alt={p.name} className="h-32 w-full object-cover" />
            <div className="space-y-1 p-3">
              <p className="line-clamp-1 text-sm font-semibold text-[#1f241f]">{p.name}</p>
              <p className="text-xs text-[#6E7F63]">{p.price ? formatPrice(p.price) : "A consultar"}</p>
              <div className="flex items-center justify-between pt-1">
                ...
                <button className="inline-flex items-center gap-1 text-[11px] font-medium text-[#6E7F63]">
      ...
      {services.length ? (
        <>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {services.map((s) => (
              <article key={s.id} className="rounded-2xl border border-[#E6DDD0] bg-white p-3">
                <p className="text-sm font-semibold text-[#1f241f]">{s.name}</p>
                <p className="mt-1 line-clamp-2 text-xs text-[#666C60]">{s.description}</p>
                <div className="mt-2 flex items-center justify-between">
                  <span className="rounded-full bg-[#E7F9EE] px-2 py-0.5 text-[10px] font-medium text-[#197a43]">
                    Activo
                  </span>
                  <button className="text-[11px] font-medium text-[#6E7F63]">Editar</button>
                </div>
              </article>
            ))}
          </div>
