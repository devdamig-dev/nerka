...
export default function EntrepreneurProfilePage() {
  ...
  return (
    <main className="pb-24 lg:px-6 lg:py-6">
      <img src={entrepreneur.cover} alt={entrepreneur.name} className="h-56 w-full object-cover lg:rounded-3xl" />

      <div className="px-4 pb-4 lg:grid lg:grid-cols-[1fr_320px] lg:gap-5 lg:px-0">
        <section>
          <img src={entrepreneur.avatar} alt={entrepreneur.name} className="-mt-10 h-20 w-20 rounded-2xl border-4 border-[#FAFAFC] object-cover lg:mt-4" />
          ...
          <section className="mt-4">
            {tab === "Trabajos" ? (
              <div className="grid grid-cols-3 gap-2 lg:grid-cols-4">
                {entrepreneurs.slice(0, 8).map((e) => <img key={e.id} src={e.cover} alt="Trabajo" className="h-24 w-full rounded-xl object-cover lg:h-28" />)}
              </div>
            ) : null}
            {tab === "Catálogo" ? (
            ...
            {tab === "Info" ? (
              <article className="space-y-2 rounded-2xl border border-[#ece8f7] bg-white p-4 text-sm text-[#433d56]">
                ...
                <p><strong>Zonas de cobertura:</strong> {entrepreneur.coverage.join(", ")}</p>
                <p><strong>Modalidad:</strong> {entrepreneur.serviceMode}</p>
        ...
        <aside className="mt-4 hidden lg:block">
          <div className="sticky top-6 rounded-2xl border border-[#ece8f7] bg-white p-4 shadow-sm">
            <p className="text-sm font-semibold text-[#2B174F]">Resumen del emprendimiento</p>
            <p className="mt-2 text-sm text-[#6F6A7C]">Cobertura: {entrepreneur.coverage.join(", ")}</p>
            <p className="text-sm text-[#6F6A7C]">{entrepreneur.serviceMode}</p>
            <Link href="/nerka/solicitudes/nueva" className="mt-4 block rounded-xl bg-[#5B2EFF] px-4 py-3 text-center text-sm font-medium text-white">Solicitar presupuesto</Link>
          </div>
        </aside>
      </div>

      <div className="fixed inset-x-0 bottom-20 z-30 mx-auto w-full max-w-lg px-4 lg:hidden">
